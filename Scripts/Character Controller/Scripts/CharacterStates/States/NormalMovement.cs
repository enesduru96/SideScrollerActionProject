...
public class NormalMovement : CharacterState
{
    ...
    public override void CheckExitTransition()
    {
        ...
        else if (!CharacterActor.IsGrounded)
        {
            ...
            if (CharacterActor.NotOnLedgeTime >= 0.4f)
                CharacterStateController.EnqueueTransition<LedgeHanging>();
        }
    }
    ...
    protected virtual void ProcessPlanarMovement(float dt)
    {
        ...
        Vector2 rawInput = new Vector2(
            normalizedInput.x != 0 ? Mathf.Sign(normalizedInput.x) : 0,
            normalizedInput.y != 0 ? Mathf.Sign(normalizedInput.y) : 0
        );

        var movementDirection = CharacterStateController.InputMovementReference * rawInput;
    ...
    JumpResult CanJump()
    {
        ...
        switch (CharacterActor.CurrentState)
        {
            case CharacterActorState.StableGrounded:
                ...
                if (CharacterActions.jump.Started && groundedJumpAvailable)
            ...
            case CharacterActorState.UnstableGrounded:
                ...
                if (CharacterActions.jump.Started && verticalMovementParameters.canJumpOnUnstableGround)
                    jumpResult = JumpResult.Grounded;
