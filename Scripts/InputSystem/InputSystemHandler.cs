...
public class InputSystemHandler : InputHandler
{
    ...
    protected virtual void Awake()
    {
        ...
        {
            string bindingGroup = "";
            for (int i = 0; i < inputActionsAsset.controlSchemes.Count; i++)
            {
                if (inputActionsAsset.controlSchemes[i].name == controlSchemeName)
                {
                    bindingGroup = inputActionsAsset.controlSchemes[i].bindingGroup;
                    break;
                }
            }
